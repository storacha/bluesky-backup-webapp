...
function EncryptionButton({
  ...
  return (
    <LockClosedIcon
      ...
      onClick={() => {
        setState(Boolean(selectedKey && !state))
      }}
    />
  )
}
...
export default function BackupUI() {
  ...
  const repo = useLiveQuery(
    () =>
      currentBackupId ? backupMetadataStore.getRepo(currentBackupId) : null,
    [currentBackupId]
  )
  const prefsDoc = useLiveQuery(
    () =>
      currentBackupId ? backupMetadataStore.getPrefsDoc(currentBackupId) : null,
    [currentBackupId]
  )
  const blobs = useLiveQuery(
    () =>
      currentBackupId ? backupMetadataStore.listBlobs(currentBackupId) : null,
    [currentBackupId]
  )
  ...
  async function onClickBackupRepo() {
    ...
    if (
      ...
      await backupRepo(
        ...
        {
          eventTarget: backupEvents,
          encryptionKey: encryptRepo ? selectedKey : undefined,
        }
      )
      setIsBackingUpRepo(false)
    } else {
      console.log(
        'not backing up, profile, agent, client:',
        bluesky.userProfile,
        bluesky.agent,
        storacha.client
      )
  ...
  async function onClickBackupPrefsDoc() {
    ...
    if (
      ...
      await backupPrefs(
        ...
        {
          eventTarget: backupEvents,
          encryptionKey: encryptPrefsDoc ? selectedKey : undefined,
        }
      )
  ...
  async function onClickBackupBlobs() {
    ...
    if (
      ...
      await backupBlobs(
        ...
        {
          eventTarget: backupEvents,
          encryptionKey: encryptBlobs ? selectedKey : undefined,
        }
      )
...
export function BackupUIView({
  ...
  const userAuthenticatedToBothServices =
    bluesky.userProfile && storacha.accounts[0]
  ...
  backupEvents.addEventListener('repo:fetching', () => {
    setBackupProgressComponent(<>Backing up your Bluesky account...</>)
  })
  backupEvents.addEventListener('repo:fetching', () => {
    setBackupProgressComponent(<>Backing up your Bluesky account...</>)
  })
  backupEvents.addEventListener('repo:uploaded', () => {
    setBackupProgressComponent(<>Repo uploaded...</>)
  })
  backupEvents.addEventListener('prefs:fetching', () => {
    setBackupProgressComponent(<>Fetching preferences...</>)
  })
  backupEvents.addEventListener('prefs:uploading', () => {
    setBackupProgressComponent(<>Backing up preferences...</>)
  })
  backupEvents.addEventListener('prefs:uploaded', () => {
    setBackupProgressComponent(<>Preferences backed up!</>)
  })
  backupEvents.addEventListener('blob:fetching', (e) => {
    const { i: loaded, count: total } = (e as CustomEvent).detail ?? {
      i: 0,
      count: 1,
    }
      ...
      {userAuthenticatedToBothServices ? (
        <div className="flex flex-col items-center">
          {space ? (
              ...
              <div className="flex flex-col items-center w-xl">
                <div className="flex flex-row justify-between items-center mb-4 space-x-8">
                  <h3 className="uppercase font-bold">
                    Creating Backup #{currentBackupId}
                  </h3>
                  ...
                  <div className="flex flex-col gap-1">
                    <h4 className="uppercase text-xs font-bold text-right">
                      Storacha Space
                    </h4>
                    <SpaceFinder
                      selected={space}
                      setSelected={setSelectedSpace}
                      spaces={storacha.spaces}
                ...
                <div className="prose-sm text-center mb-4">
                  <p>
                    Use the <b>keychain</b> 🗝️ above to create a private key if
                    you&apos;d like to encrypt your data. Be sure to export 📝
                    your key and <b>save</b> it in a password manager or another{' '}
                    <b>secure</b> location!
                  </p>
                  <p>
                    If you <b>don&apos;t</b> want to encrypt 🤪, or if
                    you&apos;ve already created a key ✅, you can back up your{' '}
                    <b>ATProto Repository</b> 🔗, &ldquo;blobs&rdquo; of data
                    like images 🏞️ and videos, and preferences 🛠️{' '}
                    <b>separately</b> below ⤵️.
                  </p>
                  <p>
                    If you choose not to <b>encrypt</b> 😰, please know that
                    your data will be uploaded to a <b>public network</b>{' '}
                    replicated around the world 🌍🌏🌎 - while it can be deleted
                    from <b>Storacha&apos;s</b> 🐔 servers, once it&apos;s{' '}
                    <b>flown the coop</b> 🐣 it will be hard to ensure it&apos;s
                    really gone!
                  </p>
                  <p>
                    Encrypting at least your preferences is highly recommended
                    🤓 - they are <b>private</b> on your PDS 👁️!
                  </p>
                </div>
                <div className="flex flex-col">
                  ...
                  <div className="flex flex-row items-center my-2 space-x-8">
                    ...
                    <div className="flex flex-row items-center">
                      <div
                        className={`${repo ? 'border-emerald-500 text-emerald-500' : 'border-gray-500 text-gray-500'} rounded-full hover:bg-white border w-8 h-8 flex flex-col justify-center items-center`}
                      >
                        <CircleStackIcon className="w-4 h-4" />
                      </div>
                      {repo && encryptRepo && (
                  ...
                  <div className="flex flex-row items-center my-2 space-x-8">
                    ...
                    <div className="flex flex-row items-center">
                      <div
                        className={`${blobs && blobs.length > 0 ? 'border-emerald-500 text-emerald-500' : 'border-gray-500 text-gray-500'} rounded-full hover:bg-white border w-8 h-8 flex flex-col justify-center items-center`}
                      >
                  ...
                  <div className="flex flex-row items-center my-2 space-x-8">
                    ...
                    <EncryptionButton
                      state={encryptPrefsDoc}
                      setState={setEncryptPrefsDoc}
                      selectedKey={selectedKey}
                    />
                    {isBackingUpPrefsDoc ? (
                      <Button
                    ...
                    <div className="flex flex-row items-center">
                      <div
                        className={`${prefsDoc ? 'border-emerald-500 text-emerald-500' : 'border-gray-500 text-gray-500'} rounded-full hover:bg-white border w-8 h-8 flex flex-col justify-center items-center`}
                      >
                        <AdjustmentsHorizontalIcon className="w-4 h-4" />
                      </div>
            ...
            <div>
              <p>
                Please choose the Storacha space where you&apos;d like to back
                up your Bluesky account:
              </p>
              <SpaceFinder
                selected={space}
                setSelected={setSelectedSpace}
            ...
            <div className="w-full flex flex-col gap-4 justify-center items-center">
              <p className="text-md text-center max-w-xl">
                You are logged in to Storacha, but we could not find any
                Storacha Spaces.
              </p>
