          ...
          <tbody>
            {backups?.map((backup) => (
              <tr key={backup.id} className="odd:bg-gray-100/80">
                <td>
                  <a href={`/backups/${backup.id}`}>{backup.id}</a>
                </td>
                <td>
          ...
          <tbody>
            {repo && (
              <tr key={repo.cid} className="odd:bg-gray-100/80">
                ...
                <td>{shortenDID(repo.accountDid)}</td>
                <td>{repo.commit}</td>
              </tr>
            )}
          </tbody>
          ...
          <tbody>
            {prefsDoc && (
              <tr key={prefsDoc.cid} className="odd:bg-gray-100/80">
                ...
                <td>{shortenDID(prefsDoc.accountDid)}</td>
              </tr>
            )}
          </tbody>
        </table>
          ...
          <tbody>
            {blobs?.map((blob) => (
              <tr key={blob.cid} className="odd:bg-gray-100/80">
                ...
                <td>{shortenDID(blob.accountDid)}</td>
              </tr>
            ))}
          </tbody>
        </table>
