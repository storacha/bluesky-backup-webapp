...
export async function backupRepo(
  ...
  const storachaUploadCid = await storachaClient.uploadCAR(
    new Blob([repoRes.data]),
    {
      onShardStored: (carMetadata) => {
        // So...what if there are multiple shards?
  ...
  if (storachaRepoCid) {
    ...
    console.warn(
      'Uploaded CAR but did not find a CID, this is very surprising and your backup cannot be recorded!'
    )
  }
...
export async function backupPrefs(
  ...
  const blob = new Blob([JSON.stringify(prefs.data)], {
...
export async function backupBlobs(
  ...
  do {
    ...
    for (const cid of listedBlobs.data.cids) {
      ...
      const storachaBlobCid = await storachaClient.uploadFile(
